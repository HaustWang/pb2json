...
public:
	...
	static test1 ToMessage(nlohmann::json const& json) {
		test1 msg;
		if(json["test_1"].is_number())	msg.set_test_1(json["test_1"].get<int32_t>());

		if(json["test_2"].is_number())	msg.set_test_2(json["test_2"].get<int32_t>());

		if(json["test_3"].is_string())	msg.set_test_3(json["test_3"].get<string>());
...
public:
	...
	static test2 ToMessage(nlohmann::json const& json) {
		...
		if (!json["test_1"].is_array()) {
			if(json["test_1"].is_object())	msg.add_test_1()->CopyFrom(Test1Translator::ToMessage(json["test_1"]));
		} else {
			int count = json.count(test_1);
			for(auto i = 0; i < count; ++i) {
				if(json["test_1"][i].is_object())	msg.add_test_1()->CopyFrom(Test1Translator::ToMessage(json["test_1"][i]));
			}
		}

		if(json["test_2"].is_string())	msg.set_test_2(json["test_2"].get<string>());

		if (!json["test_3"].is_array()) {
			if(json["test_3"].is_number())	msg.add_test_3(json["test_3"].get<int64_t>());
		} else {
			int count = json.count(test_3);
			for(auto i = 0; i < count; ++i) {
				if(json["test_3"][i].is_number())	msg.add_test_3(json["test_3"][i].get<int64_t>());
			}
		}

		if (!json["test_6"].is_array()) {
			if(json["test_6"].is_number())	msg.add_test_6(json["test_6"].get<int>());
		} else {
			int count = json.count(test_6);
			for(auto i = 0; i < count; ++i) {
				if(json["test_6"][i].is_number())	msg.add_test_6(json["test_6"][i].get<int>());
			}
		}
